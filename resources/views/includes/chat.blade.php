...
<div class="modal modal-chat fade" id="chatModal" 
        ...
        data-demand    ="" 
        data-conv_id   ="" 
        data-user      = '{"id":"{{Auth::user()->id}}","name":"{{Auth::user()->name}}"}'
        data-post-conv = "{{URL::to('/messages/conversa')}}"
        data-post-url  = "{{URL::to('/messages/insert')}}"
        data-get-url   = "{{URL::to('/messages/json')}}"
